...
public class Interpreter {
	...
	public HashMap<FrmTemp, Object> temps = new HashMap<FrmTemp, Object>(); //Temp iz Frame
	...
	public Interpreter(FrmFrame frame, ImcSEQ code) {
		...
		fp = (Integer) ldM(fp + frame.sizeFPRA);
		sp = sp + frame.size();
		if (debug) {
			System.out.println("[FP=" + fp + "]");
			System.out.println("[SP=" + sp + "]");
	...
	public Object execute(ImcCode instruction) {
		...
		if (instruction instanceof ImcCALL) {
			...
			if (instr.label.name().equals("Lsys::getInt")) {
				...
				stM((Integer) ldM (sp),scanner.nextInt()); // sp+0
				return null;
			}
			...
			if (instr.label.name().equals("Lsys::getString")) {
				...
				return null;
                    ...
                    //Interpreter interpreter = new Interpreter(compiler.frames.FrmDesc.getFrame(instr.label), (ImcSEQ) compiler.lincode.CodeGenerator.codesByLabel.get(instr.fun));
			return null;
		}
		...
		if (instruction instanceof ImcCONST) {
			...
			if(instr.value instanceof Integer)
